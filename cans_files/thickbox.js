...
function tb_showIframe(){
	$('select').hide(); //IE6 fix to hide select tags so they don't appear above iframe content
	$("#TB_load").remove();
	$("#TB_window").css({display:"block"});	
	var theframes = document.getElementsByTagName('iframe');
	for(var i = 0; i < theframes.length; i++)
		{
			theframes[i].setAttribute("allowTransparency","true");	 //IE fix to allow iframe transparency
		}	
}
...
function tb_remove() {
	...
	//$("#TB_window").fadeOut(100,function(){
	
	$('#TB_iframeContent').attr("src","about:blank"); //IE Fix - Changes src of top level iframe before closing
	...
	$('select').show(); //IE6 Fix - Shows select tags after iframe closes
